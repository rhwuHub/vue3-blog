        ...
        <div id="card-section">
          ...
          <div id="card3" class="manage-card">
            <el-upload
              ...
              action="http://139.9.220.169:9090/file/addFile"
...
const create = () => {
  axios
    .get('http://139.9.220.169:9090/user/article/page?pageNo=1&pageSize=100&isAsc=false')
  ...
  axios
    .get('http://139.9.220.169:9090/type/page?pageNo=1&pageSize=10&isAsc=false')

    .then((res) => {
      typeList.value = res.data.data
    })
  axios
    .get('http://139.9.220.169:9090/message/getAll')
...
function addEssay() {
  ...
  axios.post(`http://139.9.220.169:9090/user/essay/add`, data)
}
create()
function changePage() {
  const pageNo = currentPage.value
  axios
    .get(`http://139.9.220.169:9090/user/article/page?pageNo=${pageNo}&pageSize=10&isAsc=false`)
    .then((res) => {
      articleList.value = res.data.data
    })
}
...
function addType() {
  axios.post(`http://139.9.220.169:9090/type/add?name=${input.value}`).then((res) => {
    create()
  })
}
...
function onDeleteArticle(row) {
  ...
  axios.post(`http://139.9.220.169:9090/admin/article/delete/${id}`).then((res) => {
    create()
  })
}
function onDeleteType(row) {
  ...
  axios.post(`http://139.9.220.169:9090/type/delete/${id}`).then((res) => {
    create()
  })
}
function onEditType(row) {
  ...
  ElMessageBox.prompt('Please input your type', 'Tip', {
    ...
    .then(({ value }) => {
      ...
      axios.post(`http://139.9.220.169:9090/type/update`, data).then((res) => {
        create()
      })
    })
    .catch(() => {
