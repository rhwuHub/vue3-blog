...
editorConfig.MENU_CONF['uploadImage'] = {
  async customUpload(file, insertFn) {
    ...
    axios.post('http://139.9.220.169:9090/file/addFile', data).then((res) => {
      const url = res.data
      const alt = '图片无法显示'
      const href = url
...
onMounted(() => {
  // 查询所有分类
  axios.get(`http://139.9.220.169:9090/type/getAll`).then((res) => {
    options.value = res.data.data
    // 回显
    if (router.currentRoute.value.query.id) {
      const id = router.currentRoute.value.query.id
      axios.get(`http://139.9.220.169:9090/user/article/${id}`).then((res) => {
...
const imgAdd = (pos: any, $file: any) => {
  ...
  axios.post('http://139.9.220.169:9090/file/addFile', formData).then((res) => {
    const url = res.data
    // pos = 1
...
function primary() {
  ...
  if (router.currentRoute.value.query.id) {
    ...
    axios.post('http://139.9.220.169:9090/admin/article/update', data)
    ...
    data.append('label', dynamicTags.value)
    data.append('type', type.value)
    axios.post('http://139.9.220.169:9090/admin/article/upload', data)
    router.push({ path: '/manage' })
  }
}
function cancel() {
