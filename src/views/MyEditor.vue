...
onMounted(() => {
  // 查询所有分类
  axios.get(`http://rhwu.fun/hsbd/type/getAll`).then((res) => {
    options.value = res.data.data
    // 回显
    if (router.currentRoute.value.query.id) {
      const id = router.currentRoute.value.query.id
      axios.get(`http://rhwu.fun/hsbd/user/article/${id}`).then((res) => {
...
const imgAdd = (pos: any, $file: any) => {
  ...
  axios.post('http://rhwu.fun/hsbd/file/addFile', formData).then((res) => {
    const url = res.data
    // pos = 1
