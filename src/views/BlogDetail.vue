...
function getComment() {
  twoList = ref([])
  // 获取到一级评论
  axios.get(`http://139.9.220.169:9090/user/comment/getOne/${source}/${id}`).then((res) => {
...
axios.get(`http://139.9.220.169:9090/user/article/${id}`).then((data) => {
...
function addComment() {
  ...
  axios.get(`https://api.usuuu.com/qq/${myQQ.value}`).then((res) => {
    ...
    axios
      .post('http://139.9.220.169:9090/user/comment/addOne', {
        source: 1,
        sourceId: id,
        img: qqImg,
        name: qqName,
...
function addTwoComment(indexId: any) {
  if (myContent.value == '') {
    ElMessage.error('请输入内容')
  ...
  axios.get(`https://api.usuuu.com/qq/${myQQ.value}`).then((res) => {
    ...
    qqImg = ` http://q2.qlogo.cn/headimg_dl?dst_uin=${myQQ.value}&spec=100`
    axios
      .post('http://139.9.220.169:9090/user/comment/addTwo', {
        source: 1,
        targetId: indexId,
        img: qqImg,
        name: qqName,
...
function showTwoComment(index: any) {
