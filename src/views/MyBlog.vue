...
export default {
  ...
  methods: {
    ...
    QueryTypeAll() {
      ...
      this.isBottom = false
      axios
        .get('http://rhwu.fun/hsbd/user/article/page?pageNo=1&pageSize=5&isAsc=false')
        .then((res) => {
    ...
    QueryByTypeId(id) {
      ...
      axios
        .get(
          `http://rhwu.fun/hsbd/user/article/page?typeId=${this.isSelect}&pageNo=1&pageSize=5&isAsc=false`
        )
        .then((res) => {
          if (res.data.data.length == 0) {
            this.isBottom = true
    ...
    changePage() {
      ...
      if (this.isSelect == -1) {
        ...
        axios
          ...
          .get(
            `http://rhwu.fun/hsbd/user/article/page?pageNo=${pageNo}&pageSize=5&isAsc=false`
          )
          .then((res) => {
        ...
        axios
          // 60.205.254.145
          .get(
            `http://rhwu.fun/hsbd/user/article/page?typeId=${isSelect}&pageNo=${pageNo}&pageSize=5&isAsc=false`
          )
          .then((res) => {
            if (res.data.data.length == 0) {
              this.isBottom = true
  ...
  async created() {
    ...
    const res = await axios.get(
      'http://rhwu.fun/hsbd/user/article/page?pageNo=1&pageSize=5&isAsc=false'
    )
